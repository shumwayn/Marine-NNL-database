...
ggplot(data = Policy_Analysis, aes (x=log(GDP), y = Polity2))+
  geom_point(data = specific, aes(x=log(GDP), y = Polity2), size = 2, shape = 19, color = "green") +
  geom_text(data = specific, aes(label= COUNTRY),size = 3.5, color="black", hjust =-.08, vjust = -1)+
  ...
  geom_point(data = explicit, aes(x=log(GDP), y = Polity2), size = 2, color = "blue")+
  #geom_text(aes(label= COUNTRY),size = 3.5, color="black", hjust =1, vjust = .5) +
  xlab("logGDP") +
  ylab("Polity2") +
  xlim (20,31.5)+
