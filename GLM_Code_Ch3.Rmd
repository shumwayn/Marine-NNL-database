...
#Ordinal Logistic Regression for just marine Policies 1-2-3
```{r}
...
Just_marine <-Policy_Analysis %>% 
  filter(marine_mech ==2 | marine_mech == 3)

Just_marine$STAGE2 = factor(Just_marine$STAGE2, levels = c("0", "1", "2", "3", "4") )

Marine_Stage <- polr(Just_marine$STAGE2 ~  Just_marine$mam_bird +Just_marine$EEZ_Protected +
                      Just_marine$OHI + log(Just_marine$GDP) + 
                      Just_marine$Polity2 + Just_marine$Richness, data = Policy_Analysis, Hess = TRUE) ##DRopped Richness and NRR not significant
summary(Marine_Stage) 
coeftest(Marine_Stage)

#confidence intervals
ci_All <- confint(Marine_Stage)
...
aov_GDP <-aov (GDP ~ marine_mech, data = Var_subset)
summary.aov(aov_GDP)
etaSquared(aov_GDP)

aov_Polity <- aov (Polity2 ~ marine_mech, data = Var_subset)
summary.aov(aov_Polity)
etaSquared(aov_Polity)

aov_OHI <- aov (OHI ~ marine_mech, data = Var_subset)
summary.aov(aov_OHI)
etaSquared(aov_OHI)

aov_Rents <- aov (Rents ~ marine_mech, data = Var_subset)
summary.aov(aov_Rents)
etaSquared(aov_Rents)

aov_EEZ <- aov (EEZ_Protected ~ marine_mech, data = Var_subset)
summary.aov(aov_EEZ)
etaSquared(aov_EEZ)

aov_mam <- aov (mam_bird ~ marine_mech, data = Var_subset)
summary.aov(aov_mam)
etaSquared(aov_mam)

aov_rich <- aov (Richness ~ marine_mech, data = Var_subset)
summary.aov(aov_rich)
etaSquared(aov_rich)

#Bonferroni Correction # Only investigating ones that were significant in aov


pairwise.t.test(Var_subset$OHI, Var_subset$marine_mech, p.adjust = "none")
pairwise.t.test(Var_subset$OHI, Var_subset$marine_mech, p.adjust = "bonferroni")
t1 <- t.test(Var_subset$OHI, Var_subset$marine_mech, paired = T)
t1

pairwise.t.test(Var_subset$GDP, Var_subset$marine_mech, p.adjust = "none")
pairwise.t.test(Var_subset$GDP, Var_subset$marine_mech, p.adjust = "bonferroni")
t2 <- t.test(Var_subset$GDP, Var_subset$marine_mech, paired = T)
t2

pairwise.t.test(Var_subset$Polity2, Var_subset$marine_mech, p.adjust = "none")
pairwise.t.test(Var_subset$Polity2, Var_subset$marine_mech, p.adjust = "bonferroni")
t3 <- t.test(Var_subset$Polity2, Var_subset$marine_mech, paired = T) #to get Tvalue
t3
```

#Bonferonni correction - post hoc analysis
```{r}

#library(car)
#library(carData)

#outlierTest(lm(marine_mech ~EEZ_Protected, data=Var_subset))
...
ggplot(data = Mar_Pol, aes (x=log(GDP), y = Richness))+
  geom_point(data = Mar_Pol, aes(x=log(GDP), y = Richness, size = 2, color = "blue")) +
