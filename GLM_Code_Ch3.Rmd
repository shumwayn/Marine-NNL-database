...
#Ordinal Logistic regression for marine policies only ## not sure if just marine mech makes sense? Trish?
```{r}

Marine_Stage <- Policy_Analysis %>% 
  filter(marine_mech == 3| marine_mech == 2|marine_mech == 1) #add | marine_mech == 0?
  

Marine_Stage$STAGE2 = factor(Marine_Stage$STAGE2, levels = c("0", "1", "2", "3", "4") )

Marine_Stage2 <- polr(STAGE2 ~  mam_bird + EEZ_Protected + OHI + log(GDP) + 
                      Polity2 + Richness, data = Marine_Stage, Hess = TRUE) 
summary(Marine_Stage2) 
coeftest(Marine_Stage2)


#confidence intervals
ci_All <- confint(Marine_Stage2)
...
#Compute confusion table and misclassification error - model predicts 0,1 bette
predictstage = predict(Marine_Stage2,Marine_Stage)
table(Marine_Stage$STAGE2, predictstage)
mean(as.character(Marine_Stage$STAGE2) != 
       as.character(predictstage))
