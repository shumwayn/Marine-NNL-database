...
Marine_Stage <- Policy_Analysis %>% 
  filter(marine_mech == 3|2|1)
  

Marine_Stage$STAGE2 = factor(Marine_Stage$STAGE2, levels = c("0", "1", "2", "3", "4") )

All_Stage2 <- polr(Marine_Stage$STAGE2 ~  Policy_Analysis$mam_bird +Policy_Analysis$EEZ_Protected +
                      Policy_Analysis$OHI + log(Policy_Analysis$GDP) + 
                      Policy_Analysis$Polity2 + Policy_Analysis$Richness, data = Policy_Analysis, Hess = TRUE) 
summary(All_Stage2) 
coeftest(All_Stage2)
...
TukeyHSD(Marine_aov, as.factor(Polity20, Var_subset))
